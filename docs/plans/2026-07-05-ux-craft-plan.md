...
- [x] Implement; verify: move summonSpring response slider, replay reflects it without rebuild; Write Back produces a compilable file matching controls.
- [x] Commit `conn: tuning playground with write-back`.
...
Execution record 2026-07-07: DesignTokens.swift became `DesignTokenStore` (raw tokens as published vars, derived values as computed properties) with the `DesignTokens` enum forwarding every existing static name, so no call site changed. `InspectorView.swift` renders every raw motion, personality, and palette token (including the STOP 2 additions: thinkingEllipsisPeriod, thinkingDotOpacityFloor, toolChip*, islandGold, overrideCornerRadius) as sliders and color wells beside the cycler, derived values read-only, with Replay/Collapse driving the reveal tokens and Write Back writing the regenerated file to the source path (resolved by a compile-time file-path macro) and printing the spec-table diff to stdout. One file deviation from the packet list: the template and diff printer live in a new `TokenWriteback.swift`, added to the token guard's exclusion list because its template text IS the DesignTokens.swift body; the round-trip test pins the two files to each other (render of a default store must equal the file on disk, byte for byte, which is also the compilability proof). Six new tests in `TokenWritebackTests`; store-mutation-drives-statics and aliveness-0-flattens-derived are pinned live. Geometry tokens stay fixed (the island frame derives from notch metrics); the frozen panel tokens stay out of the inspector.
