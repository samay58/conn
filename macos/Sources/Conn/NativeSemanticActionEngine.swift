...
actor NativeSemanticActionEngine {
    ...
    func prepare(_ params: [String: Any]) -> [String: Any]? {
        guard let request = NativeActionRequest.parse(params) else {
        ...
        let query = NativeObservationQuery(
            ...
            pid: nil,
            includeMenu: request.operation == .menu,
            maxNodes: request.operation == .menu ? 500 : 300,
            maxDepth: 16,
        ...
        var effect = reboundDesired ?? derivedEffect(
            request: request,
            target: targetNode,
            baseline: baseline
        )
        ...
        let safeTarget = targetNode?.title
    ...
    private func requiresStableExecutionContext(
        ...
        case .openApp, .switchApp, .openURL, .clipboardWrite: return false
        }
    }
